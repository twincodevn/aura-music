  ...
  try {
     ...
     for (const api of instances) {
         try {
             ...
             console.log(`[stream] Invidious instance failed: ${api}`);
         }
     }
  } catch(e) {}
  return null;
...
export async function getStreamUrl(videoId: string): Promise<string | null> {
  ...
  try {
    ...
    console.log('[stream] yt-dlp fetching M4A stream for:', videoId);
    ...
    if (fallbackUrl) {
      ...
      console.log('[stream] Got Invidious fallback URL:', fallbackUrl.slice(0, 50) + '...');
      return fallbackUrl;
    }
    
    console.error('[stream] All stream fetchers failed for:', videoId);
    return null;
  }
}
